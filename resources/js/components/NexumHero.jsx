              ...
              {user ? (
                <div className="relative" ref={userMenuRef}>
                  <button
                    ...
                    className="rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 px-5 py-2 text-sm font-semibold hover:bg-emerald-500/30 transition-all flex items-center gap-2.5 cursor-pointer backdrop-blur-md shadow-sm focus:outline-none"
                  ...
                  >
                    ...
                    <span className="truncate max-w-[130px]">{user.name || 'Кабинет'}</span>
                    <LayoutDashboard className="w-4 h-4 text-emerald-400" />
                  </button>

                  {/* Dropdown Menu */}
                  {userMenuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-72 rounded-2xl bg-[#121214]/95 border border-white/10 p-2 text-white shadow-2xl backdrop-blur-xl z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                      {/* Current User Card */}
                      <div className="flex items-center gap-3 p-2.5 rounded-xl bg-white/5 border border-white/5 mb-1.5">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 font-bold text-sm">
                          {user.name ? user.name.charAt(0).toUpperCase() : 'U'}
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-bold text-white truncate">{user.name}</p>
                          <p className="text-xs text-neutral-400 truncate">{user.role_label || user.email}</p>
                        </div>
                      </div>
                            ...
                            {linkedAccounts.map((acc) => (
                              ...
                              >
                                ...
                                <span className="text-[10px] px-2 py-0.5 rounded-md bg-white/10 text-neutral-400 font-medium shrink-0">
                                  {acc.role === 'tutor' ? 'Репетитор' : acc.role === 'admin' ? 'Админ' : 'Ученик'}
                                </span>
                              </button>
                            ))}
                          </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <a
                    href="/diagnostic"
                    className="rounded-full bg-[#C6FF33] text-black font-extrabold px-5 py-2 text-sm hover:bg-[#d4ff59] transition-all shadow-md"
                  >
                    ИИ-Диагностика
                  </a>
                </div>
            ...
            <div className="fixed inset-0 z-40 bg-black/90 backdrop-blur-xl flex flex-col p-6 pt-24 space-y-4 md:hidden overflow-y-auto">
              ...
              {user ? (
                <div className="mt-4 pt-4 border-t border-white/10 space-y-2.5">
                  ...
                  <a
                    href={user.role === 'admin' ? '/site-admin' : '/admin'}
                    className="flex items-center justify-center gap-2.5 rounded-xl bg-emerald-600/30 border border-emerald-500/40 px-4 py-3 text-sm font-semibold text-emerald-300"
                  >
                  ...
                  <button
                    type="button"
                    onClick={handleLogout}
                    className="w-full flex items-center gap-2.5 rounded-xl bg-red-500/10 border border-red-500/20 px-4 py-2.5 text-sm font-semibold text-red-400 cursor-pointer"
                  >
                ...
                <div className="flex flex-col gap-3 pt-4 border-t border-white/10">
                  <a
                    href="/diagnostic"
                    className="rounded-full bg-[#C6FF33] py-3 text-center text-black font-extrabold text-sm"
                  >
                    Пройти ИИ-диагностику
                  </a>
                  <a
                    href="/login"
                    className="rounded-full bg-white/10 py-3 text-center text-white font-medium text-sm"
                  >
                    Войти
                  </a>
                </div>
